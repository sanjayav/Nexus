/**
 * Demo Co Sustainability Performance Data — typed structure mirroring the
...
export interface ReportSection {
  ...
  scope: 'core' | 'jv'        // core (Demo Co + subs) or JV
  tables: ReportTable[]
}
...
const financial: ReportSection = {
  ...
  tables: [{
    ...
    rows: [
      ...
      { id: 'fin-local', gri: '204-1 (2016)', label: 'Proportion of spending on local suppliers', unit: '%', values: v(81, 81, 76, 84) },
    ],
  }],
}
...
const human: ReportSection = {
  ...
  tables: [
    ...
    },
    {
      ...
      rows: [
        { id: 'hc-area-rayong', label: 'Region A', unit: 'Persons', split: 'mf', values: mf([4309, 1114], [4294, 1102], [5464, 1444], [4033, 1076]) },
        { id: 'hc-area-bangkok', label: 'Region B', unit: 'Persons', split: 'mf', values: mf([354, 623], [336, 618], [355, 618], [361, 604]) },
        { id: 'hc-area-other', label: 'Region C', unit: 'Persons', split: 'mf', values: mf([46, 3], [50, 5], [50, 5], [50, 5]) },
      ],
    },
    {
      id: 'hc-new',
...
export const DEMO_REPORT: DemoReport = {
  organisation: 'Demo Co Public Limited and Subsidiaries',
...
export function findRow(report: DemoReport, rowId: string): { section: ReportSection; table: ReportTable; row: ReportRow } | null {
...
export function flattenRows(report: DemoReport): Array<{ section: string; table: string; row: ReportRow }> {
